...
const getTileCoordsForView = (
  ...
  const firstXCoord = Math.floor(currentXOffset / TILE_SIZE) - 1; // subtracting 1 to fetch tiles outside of the view
  const tileXCoords = range(firstXCoord, firstXCoord + numberOfHorizontalTiles);

  const firstYCoord = Math.floor(currentYOffset / TILE_SIZE) - 1;
